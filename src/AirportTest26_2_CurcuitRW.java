...
public class AirportTest26_2_CurcuitRW extends Job{

	public static void main(String[] args) {
		AirportTest26_2_CurcuitRW job = new AirportTest26_2_CurcuitRW();
		job.run("param.ini");
...
//		job.run(list);
	...
	public void job(ArrayList<Object> controlParameterList) {
		...
		int tryN = Integer.parseInt(controlParameterList.get(index++).toString());
		double deltaW = Double.parseDouble(controlParameterList.get(index++).toString());

		RandomNetwork net = new RandomNetwork(100, 1.0);
		net.setNode(false);
		net.setEdge();
...
//			int step = net.N*1000;
		...
		try{
			...
			folderName = "CircuitRW_deltaW=" + deltaW;
			...
			parameterLabels.add("tryN");parameterLabels.add("deltaW");
